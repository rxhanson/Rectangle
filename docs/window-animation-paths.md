...
Final constrained placement aligns the size the application achieved, including aspect-constrained windows; a temporary position used to make room for growth cannot override that final alignment. IINA size-changing commands resize once and use the transition interval for its asynchronous aspect-ratio adjustment. Completion aligns its actual size through a position-only write, avoiding the repeated size writes that replay old positions. The pending completion follows normal cancellation and replacement rules. Position-only IINA commands still animate.

Native drag restore keeps both undersized and oversized readbacks pending. It retries after native movement makes room, or once after release, using the saved size without taking over the held drag's position.
